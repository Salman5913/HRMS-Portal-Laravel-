...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="chat-container" id="chat_container">
            ...
            <div
                class="d-flex flex-column justify-content-around mt-2 mx-4 @if(Session::get('id')== $msg->user_id){{'align-items-end'}} @else {{'align-items-start'}} @endif    ">
                <div id="username_container" class=" text-muted">
                    <span id="username">
                        @if(Session::get('id') != $msg->user_id)
                        {{$msg->name.', '}}
                        @endif
                    </span>
            ...
            @endforeach
            <input type="hidden" id="login_user_id" value="{{Session::get('id')}}">
        </div>

        <div class="m-auto mt-2 col-md-12">
            {{Form::open(['method'=>'post','id'=>'messageForm'])}}
            @csrf
            <input type="hidden" name="username" id="username_inp" value="{{Session::get('admin_name')}}">
